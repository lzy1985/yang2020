 ...
 * @author lzy
 * @version 1.0.0
 * @date 2017/9/6 上午11:56
 * @Description
 */
public class DynamicDataSourceProcessor implements BeanPostProcessor {

    private Logger log = LoggerFactory.getLogger(DynamicDataSourceProcessor.class);
    /**
     * 只读方法缓存
     */
        ...
        try {
            ...
            for (Map.Entry<String, TransactionAttribute> entry : nameMap.entrySet()) {
                ...
                if (!"*".equals(methodName)) {
                    ...
                    log.info("read/write transaction process  method:{} force read:{}", methodName, isRead);
                    readMethodMap.put(methodName, isRead);
                }
            }
        } catch (Exception e) {
            log.error("process read/write transaction error", e);
